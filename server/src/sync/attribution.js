...
export function reattributeAll(db) {
  ...
  const sales = db
    .prepare("SELECT transaction_id, src, sck, channel_id, attribution_source FROM sales WHERE attribution_source != 'manual'")
    .all();
  const upd = db.prepare('UPDATE sales SET channel_id = ?, attribution_source = ? WHERE transaction_id = ?');
  let changed = 0;
  const tx = db.transaction(() => {
    for (const s of sales) {
      const r = attributeSale(s, resolve);
